...
export default function FileShare() {
  ...
  return (
    <div className="tool-card">
      <h2 className="tool-title">File sharing</h2>
      <p className="tool-desc">Upload a file and get a shareable link. Max 50 MB.</p>
      ...
      >
        <input ref={inputRef} type="file" style={{ display: 'none' }} onChange={(e) => selectFile(e.target.files[0])} />
        {file ? (
        ...
        <div className="field-row" style={{ marginTop: '12px' }}>
          <div className="field">
            ...
            <input className="input" type="number" min={1} max={30} placeholder="Never"
              value={options.expiresIn} onChange={(e) => setOptions((o) => ({ ...o, expiresIn: e.target.value }))} />
          </div>
          <div className="field">
            <label className="label">Max downloads</label>
            <input className="input" type="number" min={1} placeholder="Unlimited"
              value={options.maxDownloads} onChange={(e) => setOptions((o) => ({ ...o, maxDownloads: e.target.value }))} />
          </div>
        </div>
      )}
      ...
      {file && !loading && (
        <button className="btn-primary" onClick={handleUpload}>Upload &amp; get link</button>
      )}
